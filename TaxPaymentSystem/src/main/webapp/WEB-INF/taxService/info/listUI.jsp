...
<head>
    ...
    <script type="text/javascript">
        ...
        //新增
        function doAdd(){
            document.forms[0].action = "${basePath}taxService/info_addUI.action";
            document.forms[0].submit();
        }
        //编辑
        function doEdit(id){
            document.forms[0].action = "${basePath}taxService/info_editUI.action?info.infoId=" + id;
            document.forms[0].submit();
        }
        //删除
        function doDelete(id){
            document.forms[0].action = "${basePath}taxService/info_delete.action?info.infoId=" + id;
            document.forms[0].submit();
        }
        //批量删除
        function doDeleteAll(){
            document.forms[0].action = "${basePath}taxService/info_deleteSelected.action";
            document.forms[0].submit();
        }
        // 异步发布信息 修改状态栏
        function doPublic(infoId,state) {
          $.ajax({
              url:"${basePath}taxService/info_publicInfo.action",
              data:{"info.infoId":infoId,"info.state":state},
              type:"post",
              success:function (msg) {
                    if ("success"==msg){
                     if(state==1){
                         $("#status_"+infoId).html("发布");
                         $("#opera_"+infoId).html('<a href="javascript:doPublic(\''+infoId+'\',0)">停用</a>');
                     }else {
                         $("#status_"+infoId).html("停用");
                         $("#opera_"+infoId).html('<a href="javascript:doPublic(\''+infoId+'\',1)">发布</a>');
                     }
                    }
              },
              error:function () {
        ...
        var url="${basePath}taxService/info_listUI.action";
    </script>
</head>
            ...
            <div class="content_info">
                ...
                <div class="t_list" style="margin:0px; border:0px none;">
                    <table width="100%" border="0">
                        ...
                        <s:iterator value="pageHelper.items" status="st">
                            <tr <s:if test="#st.odd"> bgcolor="f8f8f8" </s:if> >
                                ...
                                <td align="center" id="status_<s:property value="infoId"/>"><s:property value="state==1?'发布':'停用'"/></td>
                                <td align="center">
                                	<span id="opera_<s:property value="infoId"/>">
                                	<s:if test="state==1">
